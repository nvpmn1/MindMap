...
import type {
  ...
  AIAgentAction,
  ViewMode,
} from '../components/mindmap/editor/types';
import {
  useEditorState,
  useNodeOperations,
  useMapPersistence,
...
import {
  ...
  STATUS_CONFIG,
  PRIORITY_CONFIG,
} from '../components/mindmap/editor/constants';
      ...
      <div className="flex items-center gap-3 mb-4 sticky top-0 bg-[#060910] z-10 pb-3 border-b border-white/5">
        ...
        >
          ...
          {Object.entries(NODE_TYPE_CONFIG).map(([k, c]) => (
            <option key={k} value={k} className="bg-[#111827]">
              {c.label}
            </option>
          ))}
        </select>
        <select
          ...
          className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-3 py-2 text-sm text-slate-300 outline-none"
        >
          <option value="label" className="bg-[#111827]">
            Nome
          </option>
          <option value="type" className="bg-[#111827]">
            Tipo
          </option>
          <option value="status" className="bg-[#111827]">
            Status
          </option>
          <option value="priority" className="bg-[#111827]">
            Prioridade
          </option>
        </select>
      </div>
      ...
      <div className="space-y-1">
        {sortedNodes.map((node) => {
          ...
          return (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-gradient-to-br ${cfg.gradient} border ${cfg.borderColor}`}
              >
              ...
              <span className={`text-[9px] px-2 py-0.5 rounded-full ${stCfg.bg} ${stCfg.color}`}>
                {stCfg.label}
              </span>
        ...
        {Object.entries(STATUS_CONFIG).map(([status, config]) => {
          ...
          return (
            <div key={status} className="w-72 flex-shrink-0 flex flex-col h-full">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className={`ml-auto text-[10px] font-bold ${prCfg.color}`}>
                          {prCfg.icon}
                        </span>
                      </div>
                      {node.data.description && (
                      ...
                      {node.data.progress > 0 && (
                        <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${node.data.progress}%`, backgroundColor: cfg.color }}
                          />
                        </div>
                      )}
                      {node.data.tags && node.data.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {node.data.tags.slice(0, 3).map((t, i) => (
                            <span
                              key={i}
                              className="px-1.5 py-0.5 rounded text-[8px] bg-white/5 text-slate-400"
                            >
                              {t}
                            </span>
                          ))}
              ...
              <div
                className="mt-2 py-2 border-2 border-dashed border-white/[0.06] rounded-xl text-center text-[10px] text-slate-600 hover:border-cyan-500/30 transition-colors"
                onDragOver={(e) => {
                  e.preventDefault();
                ...
                onDrop={(e) => {
                  ...
                  if (nid) {
                    onUpdateNodeData(nid, { status: status as any });
                    toast.success(`Movido para ${config.label}`);
                  }
                }}
              >
                Solte aqui
...
const TimelineView: React.FC<{
  ...
  const sortedNodes = useMemo(
    () =>
      [...nodes].sort((a, b) =>
        (a.data.dueDate || a.data.createdAt || '9999').localeCompare(
          b.data.dueDate || b.data.createdAt || '9999'
        )
      ),
    [nodes]
  );
        ...
        {sortedNodes.map((node) => {
          ...
          return (
            <div key={node.id} className="relative flex items-start gap-4 mb-6 ml-2">
              <div
                className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 border-2 transition-all ${selectedNodeId === node.id ? 'bg-cyan-500/20' : 'bg-[#0c1220]'}`}
                style={{
                  borderColor: selectedNodeId === node.id ? cfg.color : 'rgba(255,255,255,0.1)',
                }}
              >
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span
                    className={`text-[9px] px-2 py-0.5 rounded-full ${stCfg.bg} ${stCfg.color}`}
                  >
                    {stCfg.label}
                  </span>
                </div>
                {node.data.description && (
                  <p className="text-[11px] text-slate-500 line-clamp-2">{node.data.description}</p>
                )}
                {node.data.dueDate && (
...
function NeuralMapEditorInner() {
  ...
  const { createNode, updateNodeData, deleteNode, duplicateNode, onConnect } = useNodeOperations(
    nodes,
    edges,
    setNodes,
    setEdges,
  ...
  const [linkingSourceNode, setLinkingSourceNode] = useState<string | null>(null);

  const startConnection = useCallback((nodeId: string, position: string) => {
    setLinkingSourceNode(nodeId);
    document.body.style.cursor = 'crosshair';
  }, []);

  const confirmConnection = useCallback(
    (targetNodeId: string) => {
      if (!linkingSourceNode || !targetNodeId) {
        return;
      }
      ...
      // Use the existing onConnect from useNodeOperations
      onConnect({
        source: linkingSourceNode,
        target: targetNodeId,
        sourceHandle: null,
        targetHandle: null,
      } as any);
  ...
  const cancelConnection = resetLinking;

  // Escape key to cancel linking
  React.useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && linkingSourceNode) {
        resetLinking();
      }
    };
    window.addEventListener('keydown', handleKeyPress);
    return () => window.removeEventListener('keydown', handleKeyPress);
  }, [linkingSourceNode, resetLinking]);
  // └────────────────────────────────────────────────────────────────────────────┘
    ...
    return nodes.map((node) => ({
      ...
      data: {
        ...
        onAIExpand: (nodeId: string) => {
          ...
          if (nd) {
            setAiPendingPrompt(
              `Expanda o tópico "${nd.data.label}" com sub-tópicos e ideias derivadas`
            );
          }
        },
        ...
        onConfirmConnection: () => confirmConnection(node.id),
        onCancelConnection: cancelConnection,
        // Connection state for styling
        isInLinkingMode: linkingSourceNode !== null,
        isLinkingSource: linkingSourceNode === node.id,
      },
    }));
  }, [
    nodes,
  ...
  const onNodeClick = useCallback(
    (_: React.MouseEvent, node: PowerNode) => {
  ...
  const onNodeContextMenu = useCallback(
    (event: React.MouseEvent, node: PowerNode) => {
      event.preventDefault();
      setSelectedNodeId(node.id);
  ...
  const handleAIQuickAction = useCallback(
    (actionType: string, nodeId: string) => {
    ...
    (action: ContextMenuAction) => {
      switch (action.type) {
        case 'add-node': {
          ...
          createNode(action.nodeType, p, selectedNodeId);
          break;
        }
        case 'delete-node':
        ...
        case 'copy-node': {
          ...
          if (n) {
            setClipboard(JSON.parse(JSON.stringify(n)));
        ...
        case 'paste-node': {
          if (clipboard) {
            const p = action.position
              ? screenToFlowPosition({ x: action.position.x, y: action.position.y })
              : { x: clipboard.position.x + 50, y: clipboard.position.y + 50 };
            createNode(clipboard.data.type, p, null, {
              ...clipboard.data,
        ...
        case 'pin-node':
          updateNodeData(action.nodeId, {
            pinned: !nodes.find((n) => n.id === action.nodeId)?.data.pinned,
          });
          toast.success('Nó fixado!', { duration: 2500 });
          break;
        case 'lock-node': {
          const locked = nodes.find((n) => n.id === action.nodeId)?.data.locked;
          updateNodeData(action.nodeId, { locked: !locked });
          setNodes((prev) =>
            prev.map((n) => (n.id === action.nodeId ? { ...n, draggable: !!locked } : n))
          );
          toast.success(locked ? 'Nó desbloqueado' : 'Nó bloqueado', { duration: 2500 });
          break;
        }
        case 'collapse-node': {
          const childIds = new Set(
            edges.filter((e) => e.source === action.nodeId).map((e) => e.target)
          );
          setNodes((prev) =>
            prev.map((n) => (childIds.has(n.id) ? { ...n, hidden: !n.hidden } : n))
          );
          setEdges((prev) =>
            prev.map((e) => (e.source === action.nodeId ? { ...e, hidden: !e.hidden } : e))
          );
          toast.success('Filhos alternados', { duration: 2500 });
          break;
        }
        case 'expand-all':
          setNodes((prev) => prev.map((n) => ({ ...n, hidden: false })));
          setEdges((prev) => prev.map((e) => ({ ...e, hidden: false })));
          toast.success('Todos expandidos', { duration: 2500 });
          break;
        case 'select-all':
        ...
        case 'group-selected': {
          ...
          if (sel.length < 2) {
            toast.error('Selecione pelo menos 2 nós', { duration: 3500 });
            break;
          }
          const gn = createNode(
            'group',
            ...
            null,
            { label: 'Grupo', description: `${sel.length} nós agrupados` }
          );
          sel.forEach((n) =>
            setEdges((prev) => [
              ...prev,
              {
                id: `edge_${gn.id}_${n.id}`,
                source: gn.id,
                target: n.id,
                type: 'power',
                animated: true,
          ...
          toast.success(`${sel.length} nós agrupados`, { duration: 2500 });
          break;
        }
        case 'ai-expand':
          setSelectedNodeId(action.nodeId);
          setShowAIPanel(true);
          handleAIQuickAction('expand', action.nodeId);
          break;
        case 'ai-analyze':
          setSelectedNodeId(action.nodeId);
          setShowAIPanel(true);
          handleAIQuickAction('analyze', action.nodeId);
          break;
        case 'ai-generate-tasks':
          setSelectedNodeId(action.nodeId);
          setShowAIPanel(true);
          handleAIQuickAction('tasks', action.nodeId);
          break;
        case 'ai-research':
          setSelectedNodeId(action.nodeId);
        ...
        case 'ai-chart':
          setSelectedNodeId(action.nodeId);
          setShowAIPanel(true);
          handleAIQuickAction('chart', action.nodeId);
          break;
        case 'change-type':
          updateNodeData(action.nodeId, { type: action.nodeType });
          break;
        case 'disconnect-all':
          saveToHistory();
          setEdges((prev) =>
            prev.filter((e) => e.source !== action.nodeId && e.target !== action.nodeId)
          );
          break;
        case 'fit-view':
          fitView({ padding: 0.2 });
      ...
      actions.forEach((action) => {
        ...
        switch (action.type) {
          ...
          case 'batch_create_nodes':
            createNode(
              (d?.type as NeuralNodeType) || 'idea',
              d?.position,
              d?.parentId || selectedNodeId,
              {
          ...
          case 'delete_edge':
            if (d?.edgeId) setEdges((prev) => prev.filter((e) => e.id !== d.edgeId));
              ...
              setEdges((prev) =>
                prev.filter((e) => !(e.source === d.source && e.target === d.target))
              );
            break;
          case 'generate_chart':
            if (selectedNodeId && d?.chartData)
              updateNodeData(selectedNodeId, { chart: d.chartData });
            break;
          case 'analyze_map':
          case 'find_nodes':
          case 'reorganize_map':
  ...
  const handleExportJSON = useCallback(() => {
    ...
    a.href = u;
    a.download = `${mapInfo?.title || 'neuralmap'}.json`;
    a.click();
  ...
  const handleExportPNG = useCallback(async () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${mapInfo?.title || 'neuralmap'}.svg`;
      a.click();
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      r.onload = (ev) => {
        try {
          ...
          if (d.nodes && d.edges) {
            saveToHistory();
            if (d.mapInfo) setMapInfo(d.mapInfo);
            setNodes(d.nodes);
            setEdges(d.edges);
  ...
  const handleShare = useCallback(async () => {
    try {
      if (navigator.share)
        await navigator.share({ title: mapInfo?.title, url: window.location.href });
      else {
  ...
  const handleInsertResearchAsNode = useCallback(
    (result: any) => {
  ...
  const handleViewSelectNode = useCallback(
    (nodeId: string) => {
      setSelectedNodeId(nodeId);
      setShowDetailPanel(true);
    },
    ...
    >
      ...
      <div className="flex-1 relative" ref={reactFlowWrapper}>
        ...
        {viewMode === 'map' && (
          <ReactFlow
            ...
            nodesDraggable={!settings.isLocked}
            nodesConnectable={!settings.isLocked}
          ...
          >
            {settings.showGrid && (
              <>
                ...
                {/* Secondary Grid: Lines for better visibility */}
                <svg
            ...
            <svg className="absolute inset-0 pointer-events-none z-[1]" width="100%" height="100%">
              <defs>
                <radialGradient id="cg1" cx="30%" cy="40%" r="50%">
                  <stop offset="0%" stopColor="rgba(6,182,212,0.03)" />
                  <stop offset="100%" stopColor="transparent" />
                </radialGradient>
                <radialGradient id="cg2" cx="70%" cy="60%" r="45%">
            ...
            <Controls
              className="!bg-[#111827]/90 !border-white/10 !rounded-xl !shadow-2xl [&>button]:!bg-transparent [&>button]:!border-white/5 [&>button]:!text-slate-400 [&>button:hover]:!text-white [&>button:hover]:!bg-white/5"
              showZoom
              showFitView
            ...
            {settings.showMinimap && (
              <MiniMap
                className="!bg-[#111827]/80 !border-white/10 !rounded-xl"
                nodeColor={(n) =>
                  NODE_TYPE_CONFIG[(n.data as NeuralNodeData)?.type || 'idea']?.color || '#64748b'
                }
                maskColor="rgba(6,9,16,0.85)"
                position="bottom-left"
                style={{ left: 60 }}
              />
            )}
            <Panel position="bottom-right" className="!m-4 !mr-6">
              <div className="flex items-center gap-3 px-3 py-1.5 rounded-lg bg-[#111827]/80 backdrop-blur-sm border border-white/[0.06] text-[10px] text-slate-500">
                ...
                {analytics.aiGeneratedNodes > 0 && (
                  <>
                    <span className="w-px h-3 bg-white/10" />
                    <span className="text-purple-400">🤖 {analytics.aiGeneratedNodes} IA</span>
                  </>
                )}
              </div>
            </Panel>
        ...
        {viewMode === 'list' && (
          <ListView
            nodes={nodes}
            edges={edges}
            onSelectNode={handleViewSelectNode}
      ...
      <style>{`
        ...
        .neural-canvas [role="presentation"] svg circle { 
          ...
          filter: drop-shadow(0 0 0.5px rgba(6,182,212,0.2));
        }
